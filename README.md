...
- **複数通貨対応**（円・ドル）
  - ドル建てサブスクリプションは実行時に自動的に円換算
  - リアルタイム為替レート取得（1時間キャッシュ）
  - 為替レート表示機能
- 有効/無効の切り替え機能
- 日次スケジュールによる自動実行
...
2. サブスクリプション名、カテゴリ、金額、通貨（円/ドル）、実行日（毎月X日）を登録
3. 登録したサブスクリプションは毎月指定日に自動的に支出として記録されます
4. **ドル建てサブスクリプション**
   - 登録時に現在の為替レートを確認できます
   - 実行時にリアルタイム為替レートで円換算されます
   - 家計簿のメモ欄に元のドル金額が記録されます（例：定期: Claude Pro ($20.00)）
5. 有効/無効の切り替えで一時的に停止することも可能
...
ドル建てサブスクリプションで使用される為替レートを確認できます：

```bash
docker exec Oeconomica_app php artisan tinker --execute="
\$service = app(\App\Services\ExchangeRateService::class);
echo '現在の為替レート: 1 USD = ' . \$service->getUsdToJpyRate() . ' JPY';
"
```

為替レートは[exchangerate-api.com](https://api.exchangerate-api.com)から取得され、1時間キャッシュされます。API取得に失敗した場合はデフォルト値（150円）が使用されます。
